...
describe("event priority", () => {
  it("classifies supervision events", () => {
    ...
    expect(getEventPriority({ ts: 1, type: "stdout", summary: "コマンド実行の確認待ち" })).toBe("urgent");
    ...
    expect(getEventPriority({ ts: 1, type: "test", summary: "テスト成功" })).toBe("progress");
  });
  ...
  it("resets progress throttling for a new session or task", () => {
    let current = 0;
    const gate = createCommentaryGate({ intervalMs: 2000, now: () => current });
    expect(gate.shouldEmit("progress")).toBe(true);
    current = 1;
