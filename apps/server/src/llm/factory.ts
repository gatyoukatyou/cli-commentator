...
import { createOpenCodeGoAdapter } from "./providers/opencode-go.js";

const VALID_PROVIDERS = ["disabled", "mock", "openai", "opencode-go", "groq", "local", "anthropic", "gemini"] as const;
...
export function createLLMAdapter(env: Record<string, string | undefined> = process.env): LLMAdapter {
  ...
  if (provider === "opencode-go") {
    return createOpenCodeGoAdapter(env);
  }
