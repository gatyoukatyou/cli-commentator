...
Set `TARGET_CMD` in `~/.config/cli-commentator/env`. Use `apps/server/.env.example` as the template and do not create a real `.env` in the worktree. Examples: `/bin/bash`, `/bin/zsh`, `powershell`, `claude`, `codex`, `hermes`
